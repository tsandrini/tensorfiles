...
rec {
  ...
  */
  groupAttrsetBySublistElems =
    ...
    inputAttrset:
    let
      flattened = concatMap (name: map (value: nameValuePair name value) inputAttrset.${name}) (
    ...
    let
      aux =
        ...
        let
          ...
          rest = tail parts;
        in
        if length rest == 0 then (hasAttr elem acc) else (hasAttr elem acc) && (aux acc.${elem} rest);
    in
