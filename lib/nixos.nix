...
with builtins; rec {
  ...
  */
  absolutePathToRelativeHome =
    ...
    ((if home != null then home else getUserHomeDir { inherit _user cfg; })
      + "/") path;
  ...
  */
  getUserHomeDir = {
    ...
    cfg ? null }:
    let fallbackValue = if _user != "root" then "/home/${_user}" else "/root";
    in if (cfg != null) then

      (if ((isUsersSystemEnabled cfg)
      ...
      else if ((hasAttr _user cfg.users.users)
        && (cfg.users.users.${_user}.home != null)) then
        cfg.users.users.${_user}.home
